$(function() {
  ...
  var url = "/find/fan/from/" + now + "/to/" + nowto ;
  document.getElementById("dates_fan").innerHTML = start;
  $.ajax({
    type: "GET",
    url:url,
    dataType: 'json',
    ...
    $("#bt_fan").click(function(){
      var from = $("#from_fan").val();
      var fromv = new Date(from).getTime();
      var to = fromv + 86400000;
      var urlAjax = "/find/fan/from/" + new Date(from).getTime() + "/to/" + to ;
      $.ajax({
        type: "GET",
        url:urlAjax,
        dataType: 'json',
        success: function(data) {
          ...
          DrawGraph_fan(data);
          document.getElementById("dates_fan").innerHTML = new Date(from);
        }
      });
      //alert(urlAjax);
...
function DrawGraph_fan(data) {
  var chartData = [];
  //console.log("Recibo: "+ data.length);
  var chart =AmCharts.makeChart("s_fan", {
    "type": "serial",
    "theme": "light",

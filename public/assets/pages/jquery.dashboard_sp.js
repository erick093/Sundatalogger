$(function() {
  ...
  var url = "/find/SP/from/" + now + "/to/" + nowto ;
  document.getElementById("dates_sp").innerHTML = start;
  $.ajax({
    type: "GET",
    url:url,
    dataType: 'json',
    ...
    $("#bt_sp").click(function(){
      var from = $("#from_sp").val();
      var fromv = new Date(from).getTime();
      var to = fromv + 86400000;
      var urlAjax = "/find/SP/from/" + new Date(from).getTime() + "/to/" + to ;
      $.ajax({
        type: "GET",
        url:urlAjax,
        dataType: 'json',
        success: function(data) {
          ...
          DrawGraph_SP(data);
          document.getElementById("dates_sp").innerHTML = new Date(from);
        }
      });
      //alert(urlAjax);
...
function DrawGraph_SP(data) {
  var chartData = [];
  //console.log("Recibo: "+ data.length);
  var chart =AmCharts.makeChart("s_sp", {
    "type": "serial",
    "theme": "light",

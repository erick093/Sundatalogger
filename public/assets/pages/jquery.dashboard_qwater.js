$(function() {
  ...
  var url = "/find/Qw/from/" + now + "/to/" + nowto ;
  document.getElementById("dates_qw").innerHTML = start;
  $.ajax({
    type: "GET",
    url:url,
    dataType: 'json',
    ...
    $("#bt_qw").click(function(){
      var from = $("#from_qw").val();
      var fromv = new Date(from).getTime();
      var to = fromv + 86400000;
      var urlAjax = "/find/Qw/from/" + new Date(from).getTime() + "/to/" + to ;
      $.ajax({
        type: "GET",
        url:urlAjax,
        dataType: 'json',
        success: function(data) {
          ...
          DrawGraph_Qw(data);
          document.getElementById("dates_qw").innerHTML = new Date(from);
        }
      });
      //alert(urlAjax);
...
function DrawGraph_Qw(data) {
  var chartData = [];
  //console.log("Recibo: "+ data.length);
  var chart =AmCharts.makeChart("s_qw", {
    "type": "serial",
    "theme": "light",

$(function() {
  ...
  var url = "/find/acp/from/" + now + "/to/" + nowto ;
  document.getElementById("dates_acp").innerHTML = start;
  $.ajax({
    type: "GET",
    url:url,
    dataType: 'json',
    ...
    $("#bt_acp").click(function(){
      var from = $("#from_acp").val();
      var fromv = new Date(from).getTime();
      var to = fromv + 86400000;
      var urlAjax = "/find/acp/from/" + new Date(from).getTime() + "/to/" + to ;
      $.ajax({
        type: "GET",
        url:urlAjax,
        dataType: 'json',
        success: function(data) {
          ...
          DrawGraph_ACP(data);
          document.getElementById("dates_acp").innerHTML = new Date(from);
        }
      });
      //alert(urlAjax);
...
function DrawGraph_ACP(data) {
  var chartData = [];
  //console.log("Recibo: "+ data.length);
  var chart =AmCharts.makeChart("s_acp", {
    "type": "serial",
    "theme": "light",

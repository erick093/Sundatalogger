$(function() {
  ...
  var url = "/find/Tint/from/" + now + "/to/" + nowto ;
  document.getElementById("dates_tint").innerHTML = start;
  $.ajax({
    type: "GET",
    url:url,
    dataType: 'json',
    ...
    $("#bt_tint").click(function(){
      var from = $("#from_tint").val();
      var fromv = new Date(from).getTime();
      var to = fromv + 86400000;
      var urlAjax = "/find/Tint/from/" + new Date(from).getTime() + "/to/" + to ;
      $.ajax({
        type: "GET",
        url:urlAjax,
        dataType: 'json',
        success: function(data) {
          ...
          DrawGraph_Tint(data);
          document.getElementById("dates_tint").innerHTML = new Date(from);
        }
      });
      //alert(urlAjax);
...
function DrawGraph_Tint(data) {
  var chartData = [];
  //console.log("Recibo: "+ data.length);
  var chart =AmCharts.makeChart("s_tint", {
    "type": "serial",
    "theme": "light",
